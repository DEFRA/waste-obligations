...
When an incomplete backfill exists and normal polling is enabled, the backfill worker runs the current-year sweep first and admits one historical read only when that sweep has no due work; it retains the current-year pacing decision. When normal polling is disabled, it processes historical work in its normal batches. The worker performs one successful calculation read per saved organisation/year, retaining the resulting summary but deactivating it from normal hydration. Transient failures remain queued with the ordinary retry back-off until a successful read. When every saved organisation/year has succeeded, the worker marks that year complete. Starting the operation again returns `AlreadyCompleted` rather than changing the frozen population or repeating its reads. `GET /admin/unsubmitted-compliance-declarations/polling-status` exposes each backfill's saved population size, requested/completed timestamps, and `Running` or `Completed` state.
