...
8. Do not add a separate reconciliation sweep. The current-year due/retry queue is the sole normal polling path; explicit historical backfill is separate and finite.

At the 1 February UK-time boundary, the worker selects the new current year only. It does not pre-warm that year or reconcile the former current year. Previous-year summaries may be retained under a normal operational retention policy and can be returned by an explicit historical-year or all-years query; their metrics are not refreshed unless an administrator starts the one-off historical backfill before that year is marked complete.
