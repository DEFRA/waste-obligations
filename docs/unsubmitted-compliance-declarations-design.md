...
During normal operation, the hydration controller derives its desired rate from the active local summary population rather than treating the safety ceiling as a fixed operating rate:
...
The normal worker never automatically hydrates a prior year, including across 1 February. At that boundary the former current-year summary is retained and the new current year begins its normal schedule; an explicit one-off backfill is the only historic hydration path.
...
It returns the current, stateful materialised picture rather than querying either downstream service: active eligibility generation/fingerprint/row count/promotion/verification version, visible and hydration-eligible counts, every materialised obligation year with its active-generation row count, reference-resolution state counts, worker lease expiry/release information, active/due/pending/ready/failed/never-successfully-read **current-year** obligation summaries, oldest due and success timestamps, the configured worker settings, and the controller's desired/effective request rate, latency/failure observations, rate-backoff reason, and estimated target gap. `eligibility.materialisedObligationYears` is the complete current eligibility generation, whereas `obligationHydration.years` contains only current-year normal-hydration summaries. Historical progress is represented separately by `obligationHydration.historicalBackfills`; it is never included in normal polling capacity or staleness estimates. `totalMinimumFullRefreshMinutes` is the current-year workload divided by the shared `MaxDownstreamRequestsPerMinute`. `estimatedFullRefreshMinutes` uses the effective controller rate, which is constrained by the safety ceiling and the observed downstream latency/concurrency capacity, and `estimatedStalenessGapMinutes` is the amount by which it exceeds `refreshIntervalSeconds`. These remain capacity signals rather than an SLA because retries, contention and changes in downstream behaviour can still affect completion time.
