...
**Status:** the initial server-side delivery is implemented in this branch: the local eligibility snapshot, Account-reference materialisation, direct unsubmitted-visibility and sortable obligation fields on those rows, unsubmitted query endpoint with generic search and indexed sorting, organisation-obligation summary hydration/polling, CloudWatch Embedded Metric Format (EMF) operational metrics, and private polling-status, polling-volume, and polling-plan endpoints. The later event-driven sections remain future design considerations. `Unsubmitted` remains an inferred review state rather than a compliance-declaration status.
...
For example, every day from 1 to 31 January 2027 has current obligation year `2026`; 1 February 2027 changes it to `2027`. The implementation uses one tested domain service with an injected `TimeProvider` and the explicit `Europe/London` business time zone, never the host-local clock. The worker reads that one current year once per run, so a run cannot straddle the February boundary with two interpretations.

Eligibility data continues to be loaded for all years, because it is the source needed to identify registrations/status transitions. The rolling obligation-hydration worker normally operates only for `currentObligationYear`, but deliberately overlaps years at the annual boundary: during January it warms the incoming year, and during the configured grace period after 1 February it reconciles outgoing-year summaries that were not successfully read since midnight. It must not otherwise call the downstream organisation-obligation calculation endpoint for arbitrary historic or future years. Summary hydration is non-blocking: it enriches rows already eligible from the snapshot and declaration state.

The endpoint permits historic and future obligation-year queries against the active eligibility and declaration-state projections. Those queries do not start historic/future polling: where a successful stored summary does not exist, the response returns `recyclingObligationsMet: null` and `obligationCoveragePercentage: null`; where a retained successful summary exists, it returns its last calculated values. January warming and the short February outgoing-year reconciliation are worker-controlled handover exceptions, not query-triggered polling. Any historical-summary retention or export policy remains separate work.

### Polling capacity planning

`GET /admin/unsubmitted-compliance-declarations/polling-plan` is a private administrator endpoint for use before either poller is enabled. It reads the current Waste Organisations population through the same all-years source adapter as the eligibility worker; it does not require a local eligibility generation or active obligation summaries.

For every source obligation year, plus the current year when it has no source registrations, it returns the registered population, a planning classification, required downstream requests per minute, the recommended rate with configured headroom, and the estimated full-refresh duration at the configured hard safety ceiling. `Current` is the normal worker's year, an earlier `HistoricalBackfill` year is a candidate for an explicit one-off backfill, and a later or already-completed `Excluded` year is not eligible for either operation.

The required and recommended rates use the configured `RefreshInterval` as the target full-refresh duration and `RecommendedRateHeadroomPercentage` (20% by default):

```text
required RPM    = ceil(potential hydration organisations / target refresh minutes)
recommended RPM = ceil(required RPM × (1 + headroom / 100))
```

`MaxDownstreamRequestsPerMinute` remains the hard safety ceiling. The endpoint intentionally does not cap the recommendation at that setting: a recommendation above the ceiling makes an unmet staleness target visible. The `warnings` field distinguishes an upstream read failure from a successful plan. Successful source counts are potential hydration counts because Account reference resolution has not run; the actual active hydration population may be smaller.

During normal operation, the hydration controller derives its desired rate from the active local summary population rather than treating the safety ceiling as a fixed operating rate. During a January warm-up or February reconciliation overlap, the active populations for both years are combined before deriving that rate:

```text
desired RPM = ceil(active hydration summaries / target refresh minutes)
actual RPM  = min(desired RPM, safety ceiling, latency-adjusted rate)
```

The target refresh interval and safety ceiling remain configuration. The controller policy is deliberately not another tuning surface: it retains the most recent ten downstream reads, establishes a latency baseline from five successful reads, reduces its rate by 20% when failures exceed 10% or sustained average latency exceeds 150% of that baseline, and recovers by 10 percentage points for healthy reads. Its controller state and next request slot are atomically stored in Mongo, so a lease handover or host restart retains both the global rate ceiling and any active backoff. The private polling-status endpoint and emitted metrics expose the desired/effective rate, observed latency and failure percentage, current backoff reason, estimated full-refresh duration, and any gap from the configured target. This makes upstream pressure and any resulting staleness shortfall visible instead of masking it by increasing request pressure.

### Historical one-off backfill

`POST /admin/unsubmitted-compliance-declarations/historical-backfill` discovers the registered, supported organisation population for every prior obligation year directly from Waste Organisations. It writes one immutable population snapshot per year to the private `_unsubmitted_organisation_historical_backfills` operational collection. Its full work queue is materialised once, then resumed from that saved queue. The dedicated worker uses the normal hydration lease, so another instance cannot make a historical and current-year downstream read concurrently.

Historical backfill and normal polling are deliberately mutually exclusive. The administration endpoint accepts a start only while `OrganisationObligationHydration:PollingEnabled` is `false`; otherwise it returns `409 Conflict`. Keep normal polling disabled until every requested year is complete, then enable it for the current year. This gives the bounded one-off operation a completion path without consuming live polling capacity or obscuring its staleness SLO. The worker performs one successful calculation read per saved organisation/year, retaining the resulting summary but deactivating it from normal hydration. Transient failures remain queued with the ordinary retry back-off until a successful read. When every saved organisation/year has succeeded, the worker marks that year complete. Starting the operation again returns `AlreadyCompleted` rather than changing the frozen population or repeating its reads. `GET /admin/unsubmitted-compliance-declarations/polling-status` exposes each backfill's saved population size, requested/completed timestamps, and `Running`, `Deferred`, or `Completed` state.

The normal worker never automatically hydrates a prior year except for the finite outgoing-year reconciliation immediately after 1 February. At that boundary it shares one paced request budget between the new current year and the outgoing summaries that missed the cutover; each successful outgoing reconciliation is deactivated from normal hydration. An explicit one-off backfill is the only historic hydration path outside that grace period.
...
OrganisationObligationSummary
  ...
  priority                       NewEligible | ScheduledRefresh | Retry
...
6. Spread the due times deterministically over each interval, for example by using a stable hash of `{ organisationId, obligationYear }` as a slot within the interval. For the initial assumption of `K = 500` and a 30-minute interval this makes about 17 calls due each minute, rather than a 500-call burst at one clock time. The worker claims small due batches under its lease and uses the Mongo-backed `OrganisationObligationRequestPacer` to atomically reserve one evenly spaced downstream slot at the shared 20-requests-per-minute limit. Low concurrency (initially two requests) separately bounds in-flight calls. New-organisation reads and retries pass through the same pacer; the limit cannot be bypassed by a separate retry path. A full batch starts the next batch without the normal idle wake delay, so pacing remains capable of 20 requests per minute.
...
8. Do not add a separate reconciliation sweep outside the annual handover. The current-year due/retry queue is the sole steady-state normal polling path; January warm-up and the finite outgoing-year reconciliation are the only handover exceptions. Explicit historical backfill is separate and finite.

During January, the worker warms the incoming year alongside the current year. At the 1 February UK-time boundary, it makes the new year current and, for the configured grace period, reconciles outgoing summaries that have not been successfully read since the cutover. The two years share one paced request budget throughout either overlap. Previous-year summaries may be retained under a normal operational retention policy and can be returned by an explicit historical-year or all-years query; after the grace period their metrics are not refreshed unless an administrator starts the one-off historical backfill before that year is marked complete.

**Side requirement — summary retention.** The unique `{ organisationId, obligationYear }` key prevents repeated polling and retries from creating duplicate state. The `isHydrationActive` flag removes ineligible rows and completed historical backfill summaries from the polling path without discarding their last metrics. When an existing current-year summary becomes eligible again, make it immediately due while retaining its attempt count, so the reactivation is fresh without allowing repeated eligibility changes to bypass retry back-off. Before long-term historical operation, agree a bounded cleanup or expiry policy for obsolete summaries; retention remains a diagnostic-data decision and does not alter the query's membership rules.
...
Each batch also reconciles eligible organisation membership before reading due work. This reconciliation logs its elapsed duration and organisation count at `OrganisationObligationHydration:ReconciliationLogLevel` (default `Debug`), or at Warning when it exceeds `OrganisationObligationHydration:ReconciliationWarningThresholdSeconds` (default 5 seconds; configurable from 1 to 86,400 seconds). Set `OrganisationObligationHydration__ReconciliationWarningThresholdSeconds` to override it through the environment. Set `OrganisationObligationHydration__ReconciliationLogLevel=Information` to temporarily capture normal reconciliation durations at Information; durations exceeding the threshold always remain Warning. These environment overrides take effect on deployment/restart. The duration includes the eligibility lookup, deactivation, reactivation, and bulk upserts; it excludes workload counts, pacing, and downstream hydration. Only completed reconciliations emit this duration log; failures use the existing worker error handling, and this threshold does not impose a timeout.
...
OrganisationObligationHydration
  ...
  RecommendedRateHeadroomPercentage // 20 by default; used by the private polling-plan recommendation
  BatchSize
...
The controller derives its normal rate from active current-year materialised summaries, rather than using the safety cap as a fixed operating rate. For target interval `T` minutes and safety factor `S` (where `S = 0.8` reserves 20% of capacity), the minimum configured cap is `activeSummaryCount / (T * S)`. At 4,245 active organisation-years, a 30-minute target with 20% headroom requires `4245 / (30 * 0.8) = 176.875`, so configure at least 177 requests/minute before allowing for downstream latency and retry demand. The private polling-status endpoint returns that current-year active count and calculated `totalMinimumFullRefreshMinutes` at the shared cap, allowing this calculation to be checked in the deployed environment. Historical backfills have a separate, deliberately lower-priority queue and are not included in normal polling capacity or staleness estimates.

With EMF enabled (`AWS_EMF_ENABLED=true` and `AWS_EMF_NAMESPACE` configured), CloudWatch receives the following operational metrics. They use `Service` as a common dimension; the eligibility outcome metric adds `Outcome`, reference coverage adds `ReferenceResolutionState`, and Account lookups add `AccountReferenceLookupType` (`DirectProducer` or `ComplianceScheme`). No organisation identifier is a metric dimension.

| Metric | Meaning |
| --- | --- |
| `OrganisationEligibilityRefreshOutcome`, `OrganisationEligibilityRefreshDuration`, `OrganisationEligibilityRefreshRowCount` | completed (`Promoted` or `Unchanged`) and failed eligibility refreshes, including the materialised generation row count. A `Promoted` outcome confirms that the generation write committed. |
...
| `OrganisationEligibilityRefreshWasteOrganisationsReadDuration`, `OrganisationEligibilityRefreshWasteOrganisationsReadFailure`, `OrganisationEligibilityRefreshWasteOrganisationsRowCount` | latency, failure count, and returned organisation count for the Waste Organisations source read. The duration histogram's `SampleCount` is source-read traffic volume. |
| `OrganisationEligibilityRefreshAccountReferenceLookupDuration`, `OrganisationEligibilityRefreshAccountReferenceLookupFailure`, `OrganisationEligibilityRefreshAccountReferenceLookupBatchSize` | latency, failure count, and lookup-key volume for Account reference batches. `AccountReferenceLookupType` distinguishes the direct-producer external-ID endpoint from the compliance-scheme Companies House endpoint. The duration histogram's `SampleCount` is Account request volume. |
...
| `OrganisationObligationHydrationMinimumFullRefreshDuration`, `OrganisationObligationHydrationRefreshInterval`, `OrganisationObligationHydrationMaxDownstreamRequestsPerMinute` | best-case duration to read every active summary at the configured cap, configured target refresh interval, and the effective PRN request-rate cap. Compare the first two with a safety margin; they make an environment-specific capacity mismatch visible before summaries become stale. |
...
It returns the current, stateful materialised picture rather than querying either downstream service: active eligibility generation/fingerprint/row count/promotion/verification version, visible and hydration-eligible counts, every materialised obligation year with its active-generation row count, reference-resolution state counts, worker lease expiry/release information, active/due/pending/ready/failed/never-successfully-read summaries for every year in the current normal-hydration window, oldest due and success timestamps, the configured worker settings, and the controller's desired/effective request rate, latency/failure observations, rate-backoff reason, and estimated target gap. `eligibility.materialisedObligationYears` is the complete current eligibility generation, whereas `obligationHydration.years` contains the current year and, during the January/February handover, the incoming or outgoing year sharing the controller rate. Historical progress is represented separately by `obligationHydration.historicalBackfills`; it is never included in normal polling capacity or staleness estimates. `totalMinimumFullRefreshMinutes` is the rolling active workload divided by the shared `MaxDownstreamRequestsPerMinute`. `estimatedFullRefreshMinutes` uses that same rolling workload and the effective controller rate, which is constrained by the safety ceiling and the observed downstream latency/concurrency capacity, and `estimatedStalenessGapMinutes` is the amount by which it exceeds `refreshIntervalSeconds`. These remain capacity signals rather than an SLA because retries, contention and changes in downstream behaviour can still affect completion time.

`GET /admin/unsubmitted-compliance-declarations/reference-resolution-issues` is the corresponding private triage endpoint. It returns every non-`Resolved` row in the active generation, ordered by resolution state and organisation identity. Each row contains the Waste Organisations registration data used for the Account lookup: organisation ID, obligation year, registration type/status, name/trading name, Companies House number, country, and resolution state. It deliberately includes `Pending`, `NotFound`, `Failed`, `Ambiguous`, and `AwaitingLookupKey`, not only Account `NotFound` responses, so operations can triage every active reference-resolution issue from the same state groups reported by polling status.

`GET /admin/unsubmitted-compliance-declarations/polling-volume` is the corresponding pre-enable planning endpoint. It has the same private admin-only access and exclusion from OpenAPI, but reads the current waste-organisations source directly rather than relying on a materialised eligibility generation. It reports the source organisation count and, for each obligation year with supported `REGISTERED` registrations, both the registration count and distinct organisation count. The latter is the potential hydration workload: duplicate supported registrations for one organisation/year are one downstream obligation read. It is deliberately an upper bound, not a prediction of hydrated rows, because Account reference resolution is not performed by this read-only endpoint.
