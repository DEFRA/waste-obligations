...
| `reprocessorExporterAgency` | Required. Both common-backend ingestion paths require it, `legacy-prns` retains it, and epr-backend requires the corresponding accreditation regulator. |
...
| `issuerNotes` | Optional. Rendered as note with a "not provided" fallback. |
| `issuerReference` | Exclude. Not rendered and stored under `Legacy` by `legacy-prns`. |
| `prnSignatory` | Optional. Rendered as authorised by; NPWD ingestion permits it to be absent. |
| `prnSignatoryPosition` | Optional. Rendered as position, with null currently mapped to an empty string. |
...
| `processToBeUsed` | Required for a valid stored PRN. Both common-backend ingestion paths require it, and epr-backend can derive it for every supported material. Treat a missing value as a data-quality failure. |
...
| `reprocessingSite` | Optional. Rendered only when the note is a PRN, not a PERN; the NPWD mapper currently stores an empty string even for PRNs. |
| `accreditationYear` | Required. Both common-backend ingestion paths require a valid year, `legacy-prns` retains it, and epr-backend requires it in the accreditation snapshot. Parse it to an integer and treat a missing or invalid value as a data-quality failure. |
...
| `createdOn` | Required as `audit.createdAt` for administrative use. Available from common backend, preserved by `legacy-prns`, and required in epr-backend storage. |
| `lastUpdatedBy` | Not rendered on specific PRN pages. |
| `lastUpdatedDate` | Required as `audit.updatedAt` for administrative use. Available from common backend, preserved by `legacy-prns`, and required in epr-backend storage. |
...
| `authorisedBy.name` / `authorisedBy.position` | Useful. These fields belong together as the authorised-by actor/person details and map cleanly from RREPW `status.authorisedBy`. NPWD PRNs can omit either value. This is more explicit than `authorisation.name`; `issuer` separately identifies the issuing organisation. | Keep a required `authorisedBy` object with nullable `name` and `position` properties. |
...
| `audit.createdAt` / `audit.updatedAt` / lifecycle event dates | Useful for administrative inspection even though the current PRN frontend does not render them. The common `audit` object groups the record-audit and PRN-event timestamps while DTO descriptions retain their different semantics. | Keep a required `audit` object. Require `createdAt` and `updatedAt`; keep `acceptedAt`, `rejectedAt`, and `cancelledAt` nullable because each event may not have happened and common-backend detail cannot supply them separately. |
...
### Date Availability, Kind And Format

Decision: include source-store and PRN status-event dates together in a required common `audit` object. `createdAt` and `updatedAt` are required because common backend, `legacy-prns`, and epr-backend all store them. `acceptedAt`, `rejectedAt`, and `cancelledAt` remain nullable because each lifecycle event may not have happened and the common-backend detail response cannot supply those events separately.
...
All six public temporal properties — `issuedAt`, `audit.createdAt`, `audit.updatedAt`, `audit.acceptedAt`, `audit.rejectedAt`, and `audit.cancelledAt` — represent instants. None is a date-only value or a Europe/London local time. Waste Obligations should return every populated value at offset zero.

Common backend persists these timestamp values during a PRN upsert:

| Persisted value | Value written | UTC origin |
| --- | --- | --- |
| `Prn.IssueDate` | The incoming source issue/authorisation time, copied without changing the clock value. RREPW maps `status.authorisedAt`; NPWD maps `IssueDate`. | Yes for both current ingestion contracts. RREPW sends a JavaScript `Date` as JSON with `Z`. The NPWD OData contract fixture also sends `IssueDate` with `Z`; Newtonsoft.Json deserialises it as a UTC `DateTime`, and the Service Bus and common-backend HTTP hops serialise it with `Z`. |
| `Prn.StatusUpdatedOn` | RREPW `status.authorisedAt` or `status.cancelledAt`; NPWD `StatusDate` or `CancelledDate`. Producer accept/reject updates instead set it to `DateTime.UtcNow`. | Yes under the same source contracts, or generated directly with `DateTime.UtcNow`. This field is not used to populate the first Waste Obligations detail response because common backend cannot distinguish all lifecycle events from it. |
| `Prn.CreatedOn` | `DateTime.UtcNow` for a new row; retained unchanged on a later upsert. The incoming source-created date is not used. | Yes, generated by common backend. |
| `Prn.LastUpdatedDate` | `DateTime.UtcNow` on every insert or update. The incoming NPWD `ModifiedOn` value is not persisted to this column. | Yes, generated by common backend. |
| `PrnStatusHistory.CreatedOn` | `DateTime.UtcNow` when common backend records the status history entry. | Yes, generated by common backend. |

The first Waste Obligations integration reads only `IssueDate`, `CreatedOn`, and `LastUpdatedDate` from that inventory. All three are therefore UTC clock values before SQL persistence. SQL Server `datetime2` preserves those clock values and discards only the redundant offset/kind metadata. It is suitable for the current common-backend convention; changing the column to an offset-aware type is not required for this endpoint.

This is a convention rather than a database constraint: common backend does not validate that an incoming `DateTime` is UTC. The inspected current RREPW and NPWD paths satisfy the convention, but a future producer must continue to send UTC or normalise to UTC before the upsert. RREPW is proven from the producing epr-backend code. The NPWD service implementation is not present in the inspected repositories, so its evidence is the integration's OData contract fixture and the complete client/queue/common-backend transport path rather than direct inspection of NPWD production code or data.

The downstream implementations establish these source representations and kind rules:

| Source and fields | Actual downstream representation | Kind or offset that reaches an adapter | Waste Obligations rule |
| --- | --- | --- | --- |
| Common backend `createdOn` and `lastUpdatedDate` | `DateTime` properties backed by SQL Server `datetime2`; repository inserts and updates use `DateTime.UtcNow`. | The stored clock values are UTC. `datetime2` stores neither an offset nor `DateTimeKind`. The `EprContext` has no `ConfigureConventions` override, date value converter, materialisation interceptor, or PRN-specific date configuration, so EF materialises database values with `DateTimeKind.Unspecified`. | Preserve the stored UTC clock value and attach UTC kind with `DateTime.SpecifyKind(value, DateTimeKind.Utc)` before constructing `DateTimeOffset`. Do not call `ToUniversalTime()` on an `Unspecified` value because that interprets it as machine-local time. |
| Common backend `issueDate` | `DateTime` backed by the same SQL Server `datetime2` mapping. RREPW ingestion copies `status.authorisedAt`; NPWD ingestion copies `IssueDate`. Both current transports deliver a `Z` value as `DateTimeKind.Utc` and pass it through unchanged before SQL persistence. | The clock value written to SQL is UTC; database reads are `Unspecified` only because `datetime2` cannot retain the kind flag. No offset conversion is required after the read. | Reattach UTC kind to the persisted clock value. This restores metadata that was lost at the SQL boundary; it does not infer or change the instant. |
| `legacy-prns` `IssueDate`, `CreatedAt`, `UpdatedAt`, `Status.CurrentStatusAt`, and status-history `CreatedOn` | C# `DateTime` values persisted as BSON dates. The service does not override the Mongo date serializer. | The configured MongoDB.Bson 3.10 default `DateTimeSerializer` has `Kind=Utc`; a serializer round-trip converts `Unspecified`, `Utc`, and `Local` inputs to `DateTimeKind.Utc`. Direct Mongo reads therefore supply UTC `DateTime` values at millisecond precision. | Construct UTC `DateTimeOffset` values from the materialised UTC values. The shared defensive helper below may still accept `Unspecified` fixtures without shifting their clock value. |
| RREPW `status.authorisedAt`, `acceptedAt`, `rejectedAt`, and `cancelledAt` | epr-backend maps stored JavaScript `Date` objects directly into the external response. JSON serialisation emits UTC ISO strings ending in `Z`; the current integration model consumes them as nullable `DateTime`. | `Z` values deserialize to `DateTimeKind.Utc` in the current .NET integration. | A future Waste Obligations RREPW adapter should model transport fields as `DateTimeOffset?`, then call `ToUniversalTime()` to produce offset zero. Do not copy the current integration's weaker `DateTime?` transport model. |
| epr-backend `createdAt`, `updatedAt`, and `status.*.at` | Required Joi `date()`/JavaScript `Date` values persisted as Mongo BSON dates. Live creation and status events use `new Date()`; route tests serialise them as UTC ISO strings with milliseconds and `Z`. | JavaScript `Date` is an instant and the HTTP representation has an explicit zero offset. | The future detail contract should return RFC 3339/ISO 8601 timestamps with `Z`. Model them as `DateTimeOffset` in Waste Obligations and normalise with `ToUniversalTime()`. |

Common backend uses the default ASP.NET Core `System.Text.Json` date converter. Consequently a PRN loaded from SQL is expected to expose its `Unspecified` `DateTime` values without `Z` or an offset, even though the stored clock values are UTC. This is the only practical effect of using `datetime2` for the first integration: the response loses the UTC marker, not the UTC clock value. A test or WireMock fixture created with `DateTimeKind.Utc` would instead serialise with `Z` and would not reproduce this production boundary. The first integration test stub should therefore use raw common-backend JSON timestamps without a suffix, for example `"2026-01-15T10:00:00"`.

Use one conversion helper for all common-backend `DateTime` values, including `issueDate`, `createdOn`, and `lastUpdatedDate`:
...
The `Local` branch is defensive for hand-built fixtures or a future transport change; it is not the expected kind after a common-backend SQL read. Keep this conversion at the source-adapter boundary so the public DTO contains only unambiguous `DateTimeOffset` values.
...
`createdAt` and `updatedAt` are required `DateTimeOffset` values. `acceptedAt`, `rejectedAt`, and `cancelledAt` are nullable `DateTimeOffset` values. For a nullable lifecycle event that has not happened or cannot be supplied by common-backend detail, return `null` rather than deriving it from a different timestamp. Treat a missing source-store timestamp as a data-quality failure. Add property-level DTO descriptions explaining the semantics and source-specific availability of every field, and emit those descriptions into OpenAPI.

The source-store fields can be populated as:

| Source | `audit.createdAt` | `audit.updatedAt` | Assessment |
| --- | --- | --- | --- |
| `epr-prn-common-backend` | `createdOn` | `lastUpdatedDate` | Available now. SQL reads are `DateTimeKind.Unspecified`; attach UTC kind without shifting the stored clock value, using `ToUtcDateTimeOffset`. |
| `legacy-prns` | `CreatedAt` | `UpdatedAt` | Available if `legacy-prns` becomes the NPWD legacy source; values are imported from common-backend raw data and materialise from BSON with `DateTimeKind.Utc`. Convert to offset-zero `DateTimeOffset`. |
| RREPW direct payload inspected in `epr-prn-integration-function` | Not available | Not available | The inspected sync/list payload is not a viable detail source for this required contract. A different RREPW detail endpoint would have to supply both values. |
| `epr-backend` | `createdAt` | `updatedAt` | Both are required in the stored PRN document. The future recipient-scoped detail contract must expose both and Waste Obligations must convert them from JavaScript/Mongo UTC dates to `DateTimeOffset`. |
...
- source-store created and updated timestamps.
...
| `recyclingProcess` | `Legacy.ProcessToBeUsed` as a nullable string in the current Mongo document. | The field sits under the provenance subdocument. Both common-backend ingestion paths require it, so a missing/null migrated value is a data-quality failure against the required public contract. |
...
| `authorisedBy.name` | `PrnSignatory`. | Return `null` when absent; NPWD ingestion permits it to be omitted. |
| `authorisedBy.position` | `PrnSignatoryPosition`. | Return `null` when absent. |
| `reprocessingSite` | `ReprocessingSite`. | Return `null` when absent or blank. It is not applicable to PERNs, and the NPWD mapper currently stores an empty string even for PRNs. |
| `accreditationNumber` | `AccreditationNumber`. | No gap. |
| `additionalNotes` | `Notes`, mapped from common-backend `IssuerNotes`. | Rename to `additionalNotes`; return `null` when absent or blank. |
| `reprocessorExporterAgency` | `ReprocessorExporterAgency`. | Required in the source ingestion contracts and retained as a required legacy root field. Treat a missing or blank value as a data-quality failure. |
| `accreditationYear` | `AccreditationYear` as nullable integer in the current Mongo document. | Potential data-quality gap where a malformed common-backend string was mapped to `null`. Waste Obligations should treat missing/null `accreditationYear` as unable to satisfy the required integer contract. |

The following `legacy-prns` root fields remain intentionally outside the Waste Obligations common schema for now: `ProducerAgency`, `Signature`, and `PackagingProducer`. They are not rendered by the current specific PRN/PERN UI and are not available as source PRN values from the inspected RREPW payload/model. `CreatedAt` and `UpdatedAt` are exposed as required properties in the `audit` object described above, using source-store audit semantics. `IssuerReference`, `PrnId`, `ExternalId`, `SourceSystemId`, and numeric `PrnStatusId` remain excluded because they are under the `Legacy` provenance subdocument.
...
- return `404` for pre-issue and non-evidence states: `draft`, `awaiting_authorisation`, `deleted`, and `discarded`; recipient-visible records must have a PRN/PERN number and issued timestamp;
...
- return required source-store `createdAt` and `updatedAt` timestamps plus nullable accepted, rejected, and cancelled timestamps from the stored status projection within the common `audit` object;
- serialise every populated timestamp as a UTC RFC 3339/ISO 8601 value ending in `Z`, matching epr-backend's current JavaScript `Date` response behaviour;
...
- expose only recipient-visible issued-evidence statuses: `awaiting_acceptance`, `accepted`, `rejected` when supported by the source, `awaiting_cancellation`, and `cancelled`;
...
| `issuedAt` | Current get returns nullable `issuedAt` because its issuer-facing route also serves pre-issue records. | Require it in the recipient contract. The recipient endpoint must return `404` for pre-issue states and treat a missing issued timestamp on a recipient-visible status as a data-quality failure. |
...
| `accreditationYear` | Current get returns `accreditationYear`; the stored accreditation snapshot requires it. | No gap. Require it in the Waste Obligations contract. |
...
| `recyclingProcess` | Current get returns `processToBeUsed`, derived for every supported material. | No gap, rename to required `recyclingProcess`. |
...
| `reprocessorExporterAgency` | Domain accreditation requires `submittedToRegulator`; current get does not return it. RREPW payload has `accreditation.submittedToRegulator`. | Missing from current get response, but present in the stored data and inspected RREPW payload. Add it to the recipient detail response and map it to the required Waste Obligations agency name. |
| `additionalNotes` | Current get returns `notes`. | No gap, rename to `additionalNotes`. |
| `audit.createdAt` | Current get returns `createdAt`; storage requires it. | No data gap. Require it and convert to Waste Obligations `DateTimeOffset` format. |
| `audit.updatedAt` | Storage requires `updatedAt`; current get does not return it. | Missing from current get response. Add it as a required field in the future recipient detail contract. |
...
On field availability, the inspected RREPW list/sync payload is not sufficient for the proposed common schema because it lacks `obligationYear` and the required source-store `createdAt` and `updatedAt` timestamps. `obligationYear` is due to be added, while epr-backend already stores the audit timestamps. The new recipient-scoped epr-backend detail endpoint must combine those values with the other fields available from the RREPW projection. If it cannot provide the full required contract, it should be treated as not fit for purpose and expanded before integration.
...
2. Confirm that the RREPW-backed listing/navigation flow and the new recipient-scoped detail endpoint will expose the same epr-backend PRN `id`, so that identity remains stable throughout the new journey.
3. Can its response expose the common-schema fields that are present in the RREPW payload or epr-backend domain but missing from the current `packagingRecyclingNoteById` response: `isExport`/`type`, issuer organisation name, accreditation number, accreditation site address, reprocessor/exporter agency, accepted/rejected/cancelled timestamps, and required `updatedAt`?
4. Confirm the delivery shape for `obligationYear` in `epr-backend`: it will be explicitly stored and returned by the PRN-by-ID response once RREPW supplies it.
5. If NPWD legacy PRNs are imported into `epr-backend`, will epr-backend support existing common-backend/legacy GUID IDs for those imported records, despite RREPW/future PRNs using ObjectId strings?
6. Will `issuedToOrganisation.registrationType` be constrained to `LARGE_PRODUCER` and `COMPLIANCE_SCHEME`, or can additional values appear? Waste Obligations will initially map only those two values and return `null` for an unrecognised value while still deriving `displayName` with the documented fallback.
...
The first common-backend-backed implementation will only return `AwaitingAcceptance`, `Accepted`, `Rejected`, and `Cancelled`, because those are the only PRN common backend evidence statuses. `AwaitingCancellation` is additionally needed for future RREPW/epr-backend compatibility.

Do not include epr-backend `draft`, `awaiting_authorisation`, `deleted`, or `discarded` in the Waste Obligations recipient PRN detail vocabulary. Those are pre-issue or non-evidence lifecycle states, and `awaiting_authorisation` does not yet guarantee the required PRN/PERN number and issued timestamp. If the future epr-backend integration encounters those states for this endpoint, return `404`.
...
| RREPW | `awaiting_authorisation` | Do not expose through this recipient endpoint. |
...
| epr-backend / epr-frontend | `awaiting_authorisation` | Do not expose through this recipient endpoint. |
...
| `epr-prn-common-backend` | [`MaterialType` enum used by obligation calculation](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.API.Common/Enums/MaterialType.cs#L3-L14), [`PrnConstants.Materials` source material strings](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.API.Common/Constants/PrnConstants.cs#L57-L69), [`PRN material mapping seed data`](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.Data/EprContext.cs#L140-L165), [`Fibre` mapping migration](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.Data/Migrations/20260226162853_AddFibreMaterialMapping.cs#L13-L19), [`Paper` and `FibreComposite` obligation combination](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.Obligation/Services/ObligationCalculatorService.cs#L187-L207), [`PrnDto` material copy from `Eprn.MaterialName`](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.API/Dto/PrnBaseDto.cs#L71-L84), [`Eprn` `DateTime` properties](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.Data/DataModels/EPRN.cs#L52-L87), [`RREPW request-to-entity date mapping`](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.API/Mappers/PrnMapper.cs#L10-L14), [`NPWD request-to-entity date mapping`](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.API/Mappers/Mappers.cs#L9-L43), [`datetime2` PRN mappings](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.Data/Migrations/20240717145045_InitialCreate.Designer.cs#L53-L86), [`UTC repository audit writes`](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.API/Repositories/Repository.cs#L478-L542), [`EprContext` model configuration](https://github.com/DEFRA/epr-prn-common-backend/blob/feature/MO-354/src/EPR.PRN.Backend.Data/EprContext.cs#L13-L44). |
| `legacy-prns` | [`LegacyPrn` date properties](https://github.com/DEFRA/legacy-prns/blob/main/src/Api/Data/Entities/LegacyPrn.cs#L35-L59), [`Mongo convention registration`](https://github.com/DEFRA/legacy-prns/blob/main/src/Api/Data/ServiceCollectionExtensions.cs#L53-L70), [`common-backend date migration mapping`](https://github.com/DEFRA/legacy-prns/blob/main/src/Api/Services/PrnCommonBackend/Mappers.cs#L26-L64). |
| `epr-prn-integration-function` | [`FetchRrepwIssuedPrnsFunction` calls `RrepwMappers.Map` and saves the mapped PRN](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Api/Functions/FetchRrepwIssuedPrnsFunction.cs#L112-L167), [`PrnService.SavePrn` posts to `api/v2/prn`](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/RESTServices/PrnBackendService/PrnService.cs#L32-L39), [`NPWD integration stub uses a Z issue date`](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Api.IntegrationTests/Stubs/NpwdApi.cs#L55-L61), [`NPWD response deserialisation`](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/Client/NpwdClient.cs#L45-L73), [`NPWD Service Bus UTC-preserving JSON hop`](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/Service/ServiceBusProvider.cs#L17-L33), [`NPWD queue deserialisation and save`](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Api/Functions/FetchNpwdIssuedPrnsFunction.cs#L111-L145), [`NPWD date mapper`](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/Mappers/NpwdPrnToSavePrnDetailsRequestMapper.cs#L19-L46), [`RREPW` status constants](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/Models/Rrepw/RrepwStatus.cs#L3-L11), [`RREPW status date model`](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/Models/Rrepw/Status.cs#L15-L25), [`RREPW` material constants](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/Models/Rrepw/RrepwMaterialName.cs#L6-L15), [`RREPW` glass recycling process constants](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/Models/Rrepw/RrepwGlassRecyclingProcess.cs#L6-L10), [`RREPW` material to common-backend PRN detail material mapping](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/Mappers/RrepwMappers.cs#L139-L158), [`RREPW` material mapping tests](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common.UnitTests/Mappers/RrepwMappersTests.cs#L90-L118), [`RREPW` stub all-material test](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common.UnitTests/RESTServices/RrepwService/StubbedRrepwServiceTests.cs#L240-L255), [`RREPW accept/reject outbound mapper`](https://github.com/DEFRA/epr-prn-integration-function/blob/main/src/EprPrnIntegration.Common/RESTServices/RrepwService/RrepwService.cs#L130-L154). |
| `epr-backend` | [`PRN status constants and transitions`](https://github.com/DEFRA/epr-backend/blob/main/src/packaging-recycling-notes/domain/model.js#L9-L70), [`PRN Joi date schema`](https://github.com/DEFRA/epr-backend/blob/main/src/packaging-recycling-notes/repository/schema.js#L61-L110), [`live PRN creation uses JavaScript Date`](https://github.com/DEFRA/epr-backend/blob/main/src/packaging-recycling-notes/routes/post.js#L184-L192), [`detail route serialises UTC dates with Z`](https://github.com/DEFRA/epr-backend/blob/main/src/packaging-recycling-notes/routes/get-by-id.test.js#L142-L165), [`external reject endpoint maps rejection to awaiting cancellation`](https://github.com/DEFRA/epr-backend/blob/main/src/packaging-recycling-notes/routes/reject.js#L32-L36), [`ledger rejected event projects to awaiting cancellation but stores rejected slot`](https://github.com/DEFRA/epr-backend/blob/main/src/packaging-recycling-notes/application/fold-prn-from-tail-events.js#L9-L32), [`external PRN mapper exposes lifecycle dates`](https://github.com/DEFRA/epr-backend/blob/main/src/packaging-recycling-notes/application/external-prn-mapper.js#L18-L38). |
| `epr-frontend` | [`PRN status constants`](https://github.com/DEFRA/epr-frontend/blob/main/src/server/common/constants/statuses.js#L1-L8), [`PRN list status grouping`](https://github.com/DEFRA/epr-frontend/blob/main/src/server/prns/list-controller.js#L64-L78), [`PRN status display mapping`](https://github.com/DEFRA/epr-frontend/blob/main/src/server/prns/helpers/get-status-config.js#L14-L38), [`awaiting-cancellation confirm flow`](https://github.com/DEFRA/epr-frontend/blob/main/src/server/prns/cancel-controller.js#L20-L78), [`cancelled success guard`](https://github.com/DEFRA/epr-frontend/blob/main/src/server/prns/cancelled-controller.js#L14-L31). |
...
The public contract is based on valid persisted PRNs from viable detail sources, not on the permissive nullability of integration client models. A required source value that is missing, blank, default, or cannot be mapped is a data-quality failure and should result in `500`; it should not be returned as `null` or a default value. Normalize blank optional strings to `null`.

Use the C# `required` member modifier for every property marked required below, including value types and nested objects, so the generated OpenAPI required-property lists match the contract. Nullable properties should still be emitted in JSON as explicit `null` values to preserve a stable response shape.

| Public property | C# type | Required in JSON | Source and mapping rule |
| --- | --- | --- | --- |
| `id` | `required string` | Yes | Common backend `externalId`; `legacy-prns` `legacy.externalId`; epr-backend `id`. Reject empty/default IDs. |
| `number` | `required string` | Yes | Required by both common-backend ingestion paths and present once an epr-backend PRN is issued. Reject missing or blank values. |
| `type` | `required string` | Yes | Always derived from required `isExport`. Values are `PRN` and `PERN`. |
| `status` | `required string` | Yes | Every store has a current status. Reject unknown or unmapped values. |
| `issuedAt` | `required DateTimeOffset` | Yes | Common/legacy `issueDate`; epr-backend issued-operation timestamp. Apply the source-specific UTC rules in [Date Availability, Kind And Format](#date-availability-kind-and-format). Pre-issue epr-backend states are not visible to the recipient endpoint. |
| `obligationYear` | `required int` | Yes | Validated by common backend and required from the future epr-backend detail endpoint. A null legacy migration value is bad data, not valid absence. |
| `accreditationYear` | `required int` | Yes | Required by both common-backend ingestion paths and the epr-backend accreditation snapshot. A null legacy migration value is bad data. |
| `decemberWaste` | `required bool` | Yes | Required in every source. |
| `material` | `required string` | Yes | Required and constrained in every source. Reject unmapped values. |
| `recyclingProcess` | `required string` | Yes | Required by common-backend ingestion and derivable for every supported epr-backend material. A null legacy value is bad data. |
| `tonnage` | `required int` | Yes | Required in every source. |
| `issuer` | `required PrnIssuer` | Yes | Always construct the issuing-organisation role object. |
| `issuer.organisationName` | `required string` | Yes | Required in common ingestion, legacy storage, and the epr-backend issuer snapshot. |
| `recipient` | `required PrnRecipient` | Yes | Always construct the recipient-organisation role object. |
| `recipient.organisationId` | `required Guid` | Yes | Required in every source and must match the route organisation. Reject `Guid.Empty`. |
| `recipient.displayName` | `required string` | Yes | Common/legacy selected name; derive from the epr-backend recipient snapshot. Reject a missing or blank result. |
| `recipient.name` | `string?` | Yes, nullable | Common backend and `legacy-prns` cannot prove that their selected display value is the legal name. Populate only from richer sources. |
| `recipient.tradingName` | `string?` | Yes, nullable | Unavailable separately from common backend and `legacy-prns`. |
| `recipient.registrationType` | `RegistrationType?` | Yes, nullable | Unavailable from common backend and `legacy-prns`; map known epr-backend values only. |
| `authorisedBy` | `required PrnAuthorisedBy` | Yes | Always construct the authorised-person role object, even when legacy actor details are unavailable. |
| `authorisedBy.name` | `string?` | Yes, nullable | Optional for NPWD PRNs; required for current RREPW/common-v2 records. |
| `authorisedBy.position` | `string?` | Yes, nullable | Optional in the relevant actor models. |
| `accreditationNumber` | `required string` | Yes | Required in every source. |
| `reprocessingSite` | `string?` | Yes, nullable | Not applicable to PERNs. The NPWD mapper currently stores an empty string even for PRNs; normalize blank to `null`. |
| `reprocessorExporterAgency` | `required string` | Yes | Required by both common-backend ingestion paths, retained by legacy, and required as `submittedToRegulator` by epr-backend. |
| `additionalNotes` | `string?` | Yes, nullable | Optional in every source. Normalize blank to `null`. |
| `audit` | `required PrnAudit` | Yes | Always construct the audit object. |
| `audit.createdAt` | `required DateTimeOffset` | Yes | Present in common backend, `legacy-prns`, and epr-backend storage. Apply the source-specific UTC rules in [Date Availability, Kind And Format](#date-availability-kind-and-format). |
| `audit.updatedAt` | `required DateTimeOffset` | Yes | Present in common backend, `legacy-prns`, and epr-backend storage; the future recipient detail endpoint must expose it. Apply the source-specific UTC rules. |
| `audit.acceptedAt` | `DateTimeOffset?` | Yes, nullable | Present only when an acceptance event exists; unavailable from common-backend detail. Source values are UTC instants. |
| `audit.rejectedAt` | `DateTimeOffset?` | Yes, nullable | Present only when a rejection event exists; unavailable from common-backend detail. Source values are UTC instants. |
| `audit.cancelledAt` | `DateTimeOffset?` | Yes, nullable | Present only when a cancellation event exists; unavailable from common-backend detail. Source values are UTC instants. |
...
- `status` should be normalised to a Waste Obligations enum/string set and exposed with `PossibleValue` attributes: `AwaitingAcceptance`, `Accepted`, `Rejected`, `AwaitingCancellation`, and `Cancelled`. Do not return display labels from the API. Pre-issue and non-evidence epr-backend states are not visible through this recipient endpoint.
- `issuedAt` should be a required offset-zero `DateTimeOffset`. The `*At` suffix makes clear that this is a timestamp and aligns it with the other temporal properties. Common-backend SQL reads materialise `issueDate` with `DateTimeKind.Unspecified`; attach UTC kind without changing its clock value. RREPW/epr-backend values carry an explicit UTC instant. The property-level DTO description should state that this is the time the PRN/PERN was issued and, for RREPW/epr-backend records, is also the time it was authorised. The comment should identify the source mapping from RREPW `status.authorisedAt` and epr-backend `status.issued.at`, and must be emitted as the OpenAPI property description.
...
- `tonnage` should be a required `int`. This is a settled cross-system contract rather than an assumption based only on the current frontend: RREPW is consumed by `epr-prn-integration-function` as nullable `int`, PRN common backend and the packaging frontend use `int`, and epr-backend validates PRN/PERN creation with `wholeTonnage().integer()`. The epr-backend create-route tests explicitly reject `10.5` with `422`. Its use of the generic JavaScript `number` type in domain annotations does not indicate that fractional PRN tonnage is supported. No decimal conversion or rounding should be introduced by the Waste Obligations adapter.
...
- `accreditationYear` should be a required integer. Both common-backend ingestion paths and the epr-backend accreditation snapshot require it. If `legacy-prns` contains `null` because migration could not parse a historical source string, that record cannot satisfy the public contract.
- `reprocessorExporterAgency` should be a required string. Both common-backend ingestion paths require it, `legacy-prns` retains it as a required root field, and epr-backend requires the corresponding accreditation regulator.
- `audit.createdAt` and `audit.updatedAt` are required `DateTimeOffset` values with source-store semantics. `audit.acceptedAt`, `audit.rejectedAt`, and `audit.cancelledAt` are nullable `DateTimeOffset` values representing conditional PRN lifecycle events. `issuedAt` already carries the RREPW/epr-backend authorised/issued timestamp, so do not duplicate it as `audit.authorisedAt`. Every populated public value must be normalised to offset zero and use Waste Obligations' existing ISO 8601 extended format, for example `2026-01-15T10:00:00+00:00`. Do not pass through upstream `DateTime`/JavaScript `Date` strings directly and do not apply Europe/London conversion in the API adapter. DTO/OpenAPI descriptions must explain each field's semantics and the source-specific availability of nullable lifecycle events.
- `recyclingProcess` should be a required string. Both common-backend ingestion paths require it, and epr-backend derives it for every supported material. `authorisedBy.name`, `authorisedBy.position`, `reprocessingSite`, and `additionalNotes` should be nullable strings. The frontend can continue applying empty-string or "not provided" fallbacks to those optional display values.
...
- `issuer` is a required object identifying the organisation that issued the PRN/PERN, with required `organisationName`. The role noun distinguishes it from `authorisedBy`, which identifies a person, while avoiding the similar-looking `issuedBy` and `authorisedBy` property names.
...
- Non-rendered upstream fields should not be added to the first public schema unless they are first-class `legacy-prns` root fields and the intended epr-backend detail source can also supply them. `obligationYear` is the exception: it is not in the inspected RREPW payload yet, but it is rendered/used by the frontend and is due to be added as a required integer. Required source-store audit dates are a second deliberate inclusion because all viable detail stores retain them. Nullable lifecycle event dates are included because their absence has a defined event-not-yet-occurred meaning. The nullable recipient organisation details are a further deliberate inclusion: epr-backend already snapshots stable name, trading-name, and registration-type values on the PRN, while older sources can honestly represent their absence.
...
| `issuedAt` | `issueDate` | Apply `ToUtcDateTimeOffset`. A SQL-backed response is expected to deserialize as `DateTimeKind.Unspecified`; attach UTC kind without shifting its clock value. |
...
| `accreditationYear` | `accreditationYear` | Parse string to required int. Throw a data-quality exception when the value is missing, blank, or not an integer. |
...
| `recyclingProcess` | `processToBeUsed` | Required string. Throw a data-quality exception when missing or blank. |
...
| `authorisedBy.name` | `prnSignatory` | Nullable string; normalize blank to `null`. |
| `authorisedBy.position` | `prnSignatoryPosition` | Nullable string; normalize blank to `null`. |
| `reprocessingSite` | `reprocessingSite` | Nullable string; normalize blank to `null`. |
| `accreditationNumber` | `accreditationNumber` | Direct. |
| `reprocessorExporterAgency` | `reprocessorExporterAgency` | Required string. Throw a data-quality exception when missing or blank. |
| `additionalNotes` | `issuerNotes` | Nullable string; normalize blank to `null`. |
| `audit.createdAt` | `createdOn` | Apply `ToUtcDateTimeOffset`; common backend wrote the value with `DateTime.UtcNow`, but SQL `datetime2` returns it with `DateTimeKind.Unspecified`. |
| `audit.updatedAt` | `lastUpdatedDate` | Apply `ToUtcDateTimeOffset`; common backend wrote the value with `DateTime.UtcNow`, but SQL `datetime2` returns it with `DateTimeKind.Unspecified`. |
...
- deserialize into an upstream service model under `src/Api/Services/PrnCommonBackend`, keeping `issueDate`, `createdOn`, and `lastUpdatedDate` as `DateTime` so suffix-free SQL-backed values retain `DateTimeKind.Unspecified` and their original clock value;
- map all three common-backend dates through `ToUtcDateTimeOffset` before constructing `Dtos.Prn`.
...
The OpenAPI schema should model `Prn.id` and the route `prnId` parameter as plain strings with no `uuid` format. The initial common-backend adapter can still require a parseable GUID internally, but that must not leak into the public schema. The generated required-property lists must match the [Property Requiredness](#property-requiredness) table, including required nested `issuer`, `recipient`, `authorisedBy`, and `audit` objects. `Prn.issuedAt`, `audit.createdAt`, and `audit.updatedAt` should be required date-times with offset. `Prn.recipient.organisationId` should be modelled as a required string with the `uuid` format, matching the organisation route parameter. `Prn.recipient.displayName` should be required; `name`, `tradingName`, and `registrationType` should be nullable, with `registrationType` exposing only `DirectProducer` and `ComplianceScheme` when present. The lifecycle-event audit properties should be nullable date-times with offset. OpenAPI property descriptions generated from DTO comments must explain each timestamp's semantics, why lifecycle-event values can be `null`, and that populated values are returned at UTC offset zero. The `issuedAt` description must state that it is also the RREPW/epr-backend authorisation time.
...
| Layer | Tests |
| --- | --- |
| `PrnCommonBackendServiceTests` | Returns mapped PRN on `200`; returns `null` on upstream `404`; returns `null` for a non-GUID `prnId` while common backend is the only source; sends `X-EPR-ORGANISATION`; sends bearer token through existing OAuth2 handler; maps upstream `issueDate` to required `issuedAt`; maps upstream `issuedByOrg` to `issuer.organisationName`; maps upstream `organisationId` to required `recipient.organisationId`; maps upstream `organisationName` only to `recipient.displayName` and returns nullable `name`, `tradingName`, and `registrationType` as `null`; throws a data-quality exception when required `obligationYear` or `accreditationYear` is missing, blank, or not an integer; throws when required `processToBeUsed` or `reprocessorExporterAgency` is missing or blank; normalizes blank optional strings to `null`; returns unavailable accepted/rejected/cancelled audit timestamps as `null`; maps `issueDate`, `createdOn`, and `lastUpdatedDate` through `ToUtcDateTimeOffset`; maps all known common-backend PRN statuses into the Waste Obligations PRN status vocabulary and rejects unknown values; maps all known common-backend PRN `materialName` values into the Waste Obligations PRN material vocabulary, including `Fibre`, and rejects unknown values. |
...
| OpenAPI snapshot | Refresh to include the `200` schema for `Prn` and verify its required-property lists exactly match the [Property Requiredness](#property-requiredness) table. Verify required `issuedAt`, `audit.createdAt`, and `audit.updatedAt`; required nested objects; `recipient.organisationId` with `uuid` format; required `recipient.displayName`; and only the documented nullable leaf fields. Verify that accepted/rejected/cancelled fields describe lifecycle events, created/updated fields describe source-store semantics, and `issuedAt` is documented as also being the RREPW/epr-backend authorisation time. |
| Integration scenario | Stub waste organisations and PRN common backend, then verify the endpoint returns every required field from the [Property Requiredness](#property-requiredness) table plus explicit `null` values for unavailable recipient enrichment and lifecycle-event dates. |
| Date-kind contract | Build the common-backend HTTP stub from raw JSON with `issueDate`, `createdOn`, and `lastUpdatedDate` values that have no suffix, matching SQL-backed `DateTimeKind.Unspecified` output. Verify the response preserves each clock value exactly and emits `+00:00`; it must not depend on the test host's local timezone. Unit-test the UTC, Unspecified, and defensive Local branches of `ToUtcDateTimeOffset`. For future source adapters, verify epr-backend/RREPW `Z` timestamps deserialize as `DateTimeOffset` and remain the same instant at offset zero; verify a `legacy-prns` Mongo read returns UTC values. |
...
| Status compatibility | Add contract-level assertions that the public PRN status possible values are `AwaitingAcceptance`, `Accepted`, `Rejected`, `AwaitingCancellation`, and `Cancelled`; epr-backend `draft`, `awaiting_authorisation`, `deleted`, and `discarded` should not be exposed through this recipient endpoint. |
...
1. Add service models for upstream PRN common backend detail response. Model `issueDate`, `createdOn`, and `lastUpdatedDate` as `DateTime`, not `DateTimeOffset`: SQL-backed common-backend responses can contain timestamps without an offset, and parsing those directly as `DateTimeOffset` would make the result depend on the Waste Obligations host timezone.
2. Add `Dtos.Prn` and its nested DTO records for `issuer`, `recipient`, `authorisedBy`, and `audit`, following the [Property Requiredness](#property-requiredness) table exactly. Use the C# `required` modifier for all required reference and value properties so OpenAPI required-property lists are correct. Declare `recipient.registrationType` as the existing nullable `Defra.WasteObligations.Api.Dtos.RegistrationType`; do not create a separate PRN registration-type enum. Add property-level DTO descriptions to nullable source-enrichment and lifecycle-event properties, covering their semantics and reason for being `null`, and confirm those descriptions flow into OpenAPI. Add descriptions distinguishing required source-store audit timestamps from nullable lifecycle events, and state that `issuedAt` is also the authorisation time for RREPW/epr-backend PRNs.
...
11. Record the `epr-backend` follow-up work to deliver the required recipient-scoped PRN detail endpoint, including enforcement of the issued-evidence status visibility defined above, mixed identity support if NPWD PRNs are imported, recipient organisation snapshot fields, complete audit datetime support, and fields missing from `packagingRecyclingNoteById`, before treating this as a common schema across all sources.
...
- Who owns delivery of the required recipient-scoped epr-backend PRN detail endpoint, and what will its final route, versioning, and authentication be?
