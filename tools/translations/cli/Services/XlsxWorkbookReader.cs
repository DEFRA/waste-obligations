...
internal static class XlsxWorkbookReader
{
    ...
    public static async Task<EmailTranslationExportData> ReadExportDataAsync(string workbookPath)
    {
        using var archive = await ZipFile.OpenReadAsync(workbookPath);
        ...
        return new EmailTranslationExportData(
            ReadTranslatorNotes(rows, header),
            rows.Where(row => row.RowNumber > header.RowNumber)
                .Select(row => new EmailTranslationRow(
                    CellValue(row, columns, "Translation key").Trim(),
                    CellValue(row, columns, "Template name").Trim(),
                    CellValue(row, columns, "Template id").Trim(),
                    CellValue(row, columns, "Field").Trim(),
    ...
    private static async Task<IReadOnlyList<string>> ReadSharedStringsAsync(ZipArchive archive)
    {
        ...
        return document.Root?.Elements(SpreadsheetNamespace + "si").Select(ReadSharedStringItem).ToArray() ?? [];
    }
    ...
    private static async Task<string> ResolveFirstWorksheetPathAsync(ZipArchive archive)
    {
        ...
        await using var workbookStream = await workbookEntry.OpenAsync();
        await using var relationshipsStream = await relationshipsEntry.OpenAsync();
        ...
        var firstSheetRelationshipId = workbook
            .Root?.Element(SpreadsheetNamespace + "sheets")
        ...
        var target = relationships
            .Root?.Elements(PackageRelationshipsNamespace + "Relationship")
        ...
        if (string.IsNullOrWhiteSpace(target))
        {
            throw new InvalidOperationException(
                $"Workbook relationship \"{firstSheetRelationshipId}\" does not point to a worksheet."
            );
        }

        return NormalizeZipPath(Path.Combine("xl", target));
    }
            ...
            .Select(row => new WorksheetRow(
                ...
                row.Elements(SpreadsheetNamespace + "c")
                    ...
                    .ToDictionary(
                        cell =>
                            ColumnNameToNumber(
                                new string(cell.Attribute("r")!.Value.TakeWhile(char.IsLetter).ToArray())
                            ),
    ...
    private static HeaderColumns FindHeader(IReadOnlyList<WorksheetRow> rows, string workbookPath)
    {
        foreach (var row in rows)
        {
            ...
            {
                return new HeaderColumns(row.RowNumber, row.Cells);
            }
        }

        throw new InvalidOperationException(
            $"Workbook \"{workbookPath}\" is missing required Translation key and Welsh columns."
        );
    }

    private static IReadOnlyDictionary<string, int> BuildHeaderColumns(HeaderColumns header)
    {
        return header.Cells.ToDictionary(cell => cell.Value.Trim(), cell => cell.Key, StringComparer.Ordinal);
    }

    private static IReadOnlyList<string> ReadTranslatorNotes(IReadOnlyList<WorksheetRow> rows, HeaderColumns header)
    {
        return rows.Where(row => row.RowNumber < header.RowNumber)
    ...
    private static string ReadCellValue(XElement cell, IReadOnlyList<string> sharedStrings)
    {
        ...
        {
            return cell.Element(SpreadsheetNamespace + "is")
    ...
    {
        return item.Descendants(SpreadsheetNamespace + "t")
            .Aggregate(string.Empty, (current, text) => current + text.Value);
    }
    ...
    private sealed record HeaderColumns(int RowNumber, IReadOnlyDictionary<int, string> Cells);
}
