...
public static class OrganisationEligibilitySnapshotContentBuilder
{
    ...
    {
        var referenceCachesByKey = referenceCaches.ToDictionary(x => new ReferenceCacheKey(
            x.OrganisationId,
            x.RegistrationType
        ));
        var rows = sourceRows
            .Select(row =>
                Materialise(
                    ...
                    referenceCachesByKey.GetValueOrDefault(
                        new ReferenceCacheKey(row.OrganisationId, row.RegistrationType)
                    )
                )
            )
    ...
    private readonly record struct ReferenceCacheKey(Guid OrganisationId, RegistrationType RegistrationType);
}
